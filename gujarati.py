...
class Gujarati_Window:
    ...
    def add_data(self):
        if self.ID.get() == "" or self.Name.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host = "localhost", username = "root", password = "aishwarya", database = "management")
                mycursor = conn.cursor()
                mycursor.execute("Insert into guj values(%s,%s,%s,%s,%s,%s)", (self.ID.get(), 
                                                                                self.Name.get(),
                                                                                self.Category.get(),
    ...
    def fetchdata(self):
        conn = mysql.connector.connect(host = "localhost", username = "root", password = "aishwarya", database = "management")
        mycursor = conn.cursor()
        mycursor.execute("select * from guj")
    ...
    def update(self):
        if self.Category.get() == "":
            ...
            conn = mysql.connector.connect(host = "localhost", username = "root", password = "aishwarya", database = "management")
            mycursor = conn.cursor()
            mycursor.execute("update guj set RecipeName = %s, Category = %s, Ingredients = %s, RecipeDetails = %s, Time = %s where RecipeId = %s",(
                                                                                                                                                    self.Name.get(),
                                                                                                                                                    self.Category.get(),
                                                                                                                                                    self.Ingredients.get(),
                                                                                                                                                    self.RecipeDetails.get(),
    ...
    def delete(self):
        ...
        if mdelete > 0: 
            conn = mysql.connector.connect(host = "localhost", username = "root", password = "aishwarya", database = "management")
            mycursor = conn.cursor()
            query = "delete from guj where RecipeId = %s"
            value = (self.ID.get(),)
