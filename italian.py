...
class Italian_Window:
    def __init__(self, root):
        ...
        self.Ingredients = Text(self.root, height=5, width=50)
        self.Ingredients.grid(row=0, column=0, padx=10)

        # Text widget for Recipe Details
        self.RecipeDetails = Text(self.root, height=5, width=50)
        self.RecipeDetails.grid(row=1, column=0, padx=10)
        ...
        categ.grid(row = 2, column = 0, sticky = W)
        ...
        combo_categ.current(0)
        combo_categ.grid(row = 2, column = 1)

        ingredients = Label(left_frame, text = "Ingredients", font = ("times new roman", 15, "bold"), bg = "rosybrown", fg = "black", padx = 4, pady = 10)
        ingredients.grid(row = 3, column = 0, sticky = W)

        ing_text = Text(left_frame, height=5, width=29, font=("times new roman", 15, "bold"))
        ing_text.grid(row=3, column=1, pady=20, padx=5, sticky=W)
        ...
        procedure_text = Text(left_frame, height=5, width=29,  font=("times new roman", 15, "bold"))
        procedure_text.grid(row=4, column=1, pady=30, padx=5, sticky=W)

        time = Label(left_frame, text = "Time", font = ("times new roman", 15, "bold"), bg = "rosybrown", fg = "black", padx = 4, pady = 10)
        time.grid(row = 5, column = 0, sticky = W)

        time_entry = ttk.Entry(left_frame, width=10, textvariable = self.Time, font = ("times new roman", 15, "bold"))
        time_entry.grid(row = 5, column = 1, sticky = W)
        ...
        combo_search = ttk.Combobox(table_frame, textvariable = self.search_var,font = ("times new roman", 14, "bold"))
        ...
        search_entry = ttk.Entry(table_frame, textvariable=self.txt_search, width=29, font = ("times new roman", 15, "bold"))
        search_entry.grid(row = 0, column = 2)

        search = Button(table_frame, text = "Search", command=self.search, font = ("times new roman", 14, "bold"), bg = "green", fg = "white", width = 8 )
        search.grid(row = 0, column = 3, padx = 15, pady=2)

        showall = Button(table_frame, text = "Show All", command=self.showall, font = ("times new roman", 14, "bold"), bg = "green", fg = "white", width = 8 )
    ...
    def add_data(self):
        if self.ID.get() == "" or self.Name.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host = "localhost", user = "root", password = "aishwarya", database = "management", auth_plugin='mysql_native_password')
                mycursor = conn.cursor()
                mycursor.execute("Insert into recipe values(%s,%s,%s,%s,%s,%s)", (self.ID.get(), 
                                                                                self.Name.get(),
                                                                                self.Category.get(),
    ...
    def getcursor(self, event = ""):
        ...
        self.Ingredients.delete("1.0", END)  # Clear existing ingredients
        self.Ingredients.insert(END, self.Ingredients)  # Insert new ingredients
        self.RecipeDetails.delete("1.0", END)  # Clear existing procedure
        self.RecipeDetails.insert(END, self.RecipeDetails)   
        self.Time.set(row[5])
    ...
    def update(self):
        if self.Category.get() == "":
            ...
            conn = mysql.connector.connect(host = "localhost", user = "root", password = "aishwarya", database = "management", auth_plugin='mysql_native_password')
            mycursor = conn.cursor()
            mycursor.execute("update recipe set RecipeName = %s, Category = %s, Ingredients = %s, RecipeDetails = %s, Time = %s where RecipeId = %s",(
                                                                                                                                                    self.Name.get(),
                                                                                                                                                    self.Category.get(),
                                                                                                                                                    self.Ingredients.get(),
                                                                                                                                                    self.RecipeDetails.get(),
    ...
    def search(self):
        conn = mysql.connector.connect(host = "localhost", user = "root", password = "aishwarya", database = "management", auth_plugin='mysql_native_password')
        mycursor = conn.cursor()
        query = "SELECT * FROM recipe WHERE {} LIKE %s".format(self.search_var.get())
        mycursor.execute(query, ('%' + self.txt_search.get() + '%',))
