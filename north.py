...
class North_Window:
    def __init__(self, root):
        ...
        combo_categ = ttk.Combobox(left_frame, font = ("times new roman", 15, "bold"), textvariable = self.Category)
    ...
    def add_data(self):
        if self.ID.get() == "" or self.Name.get() == "":
            ...
            try:
                ...
                mycursor.execute("Insert into north values(%s,%s,%s,%s,%s,%s)", (self.ID.get(), 
                                                                                self.Name.get(),
                                                                                self.Category.get(),
    ...
    def fetchdata(self):
        conn = mysql.connector.connect(host = "localhost", username = "root", password = "aish@123", database = "management")
        mycursor = conn.cursor()
        mycursor.execute("select * from north")
    ...
    def update(self):
        if self.Category.get() == "":
            ...
            conn = mysql.connector.connect(host = "localhost", username = "root", password = "aish@123", database = "management")
            mycursor = conn.cursor()
            mycursor.execute("update north set RecipeName = %s, Category = %s, Ingredients = %s, RecipeDetails = %s, Time = %s where RecipeId = %s",(
                                                                                                                                                    self.Name.get(),
                                                                                                                                                    self.Category.get(),
                                                                                                                                                    self.Ingredients.get(),
                                                                                                                                                    self.RecipeDetails.get(),
    ...
    def delete(self):
        ...
        if mdelete > 0: 
            conn = mysql.connector.connect(host = "localhost", username = "root", password = "aish@123", database = "management")
            mycursor = conn.cursor()
            query = "delete from north where RecipeId = %s"
            value = (self.ID.get(),)
